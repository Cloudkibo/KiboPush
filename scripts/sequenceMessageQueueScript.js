    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (message.queueScheduledTime.getTime() < new Date().getTime()) {
        if (message.trigger.event === 'does_not_see') {
          SequenceSubscriberMessage.findOne({messageId: message.trigger.value, subscriberId: message.subscriberId}, (err, sequenceSubscriberMessage) => {
            if (err) {
              logger.serverLog(TAG, `err in finding sequence message ${JSON.stringify(err)}`)
            } else {
