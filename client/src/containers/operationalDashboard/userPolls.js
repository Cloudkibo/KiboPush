...
class PollsInfo extends React.Component {
  ...
  searchPolls (event) {
    ...
    for (let i = 0; i < this.props.polls.length; i++) {
      if (this.props.polls[i].statement.toLowerCase().includes(event.target.value) || this.props.polls[i].statement.toUpperCase().includes(event.target.value) || this.props.polls[i].statement === event.target.value) {
