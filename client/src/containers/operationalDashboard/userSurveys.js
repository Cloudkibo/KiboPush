...
class SurveysInfo extends React.Component {
  ...
  searchSurveys (event) {
    ...
    for (let i = 0; i < this.props.surveys.length; i++) {
      ...
      if (this.props.surveys[i].title.toLowerCase().includes(event.target.value) || this.props.surveys[i].title.toUpperCase().includes(event.target.value) || this.props.surveys[i].title === event.target.value) {
