...
class TreeStructure extends React.Component {
  ...
  getChildren (id) {
    const children = this.props.data.filter((item) => item.parentId && item.parentId === id)
    let elements = []
      ...
      for (let i = 0; i < children.length; i++) {
        elements.push(
          <StyledTreeItem
            ...
            completed={this.props.blocks.find((item) => item.uniqueId === children[i].id).payload.length > 0}
          >
