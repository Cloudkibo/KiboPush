/* eslint-disable no-return-assign */
/**
 * Created by imran on 11/11/2017.
 */

import React from 'react'

class IconStackForIntegrations extends React.Component {
  render () {
    return (
        <div className='m-widget21__item' style={{display: 'flex'}}>
          <span className='m-widget21__icon'>
            <a href='#/' className={`btn btn-${this.props.iconStyle} m-btn m-btn--icon m-btn--icon-only m-btn--custom m-btn--pill`}
            style={{width: '60px', height: '60px'}}>
              <i className={`${this.props.icon} m--font-light`} style={{fontSize: 'x-large'}} />
            </a>
          </span>
          <div className='m-widget21__info' style={{marginLeft: '10px'}}>
            <span className='m-widget21__title' style={{fontWeight: 500}}>
              {this.props.title}
            </span>
            <br />
            <span className='m-widget21__sub'>
              Outgoing Data Calls:&nbsp;&nbsp;<b style={{fontWeight: 500}}> {this.props.outgoing} </b>
            </span>
            <br />
            <span className='m-widget21__sub'>
              Incoming Data Calls:&nbsp;&nbsp; <b style={{fontWeight: 500}}> {this.props.incoming} </b>
            </span>
          </div>
        </div>
    )
  }
}

export default IconStackForIntegrations
