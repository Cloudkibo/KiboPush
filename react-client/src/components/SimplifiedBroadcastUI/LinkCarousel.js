...
class LinkCarouselModal extends React.Component {
    ...
    handleUrlMetaData(data, index) {
        ...
        if (!data || !data.ogTitle) {
            let errorMsg = ''
            if (!data) {
                errorMsg = this.props.invalidMsg ? this.props.invalidMsg : 'Invalid or private website link'
    ...
    handleLinkChange(e, index) {
        ...
        this.setState({ links, cards, edited: true }, () => {
            if (links[index].valid) {
                clearTimeout(this.typingTimer)
                this.typingTimer = setTimeout(() => this.props.urlMetaData(link, (data) => this.handleUrlMetaData(data, index), (err) => this.handleUrlMetaData(null, index)), this.doneTypingInterval)
            }
        })
    }
