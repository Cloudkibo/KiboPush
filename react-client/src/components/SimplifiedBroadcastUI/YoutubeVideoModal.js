...
class YoutubeVideoModal extends React.Component {
    constructor(props) {
        super(props)
        this.defaultErrorMsg = 'Please enter a valid YouTube video link'
        ...
        console.log('YoutubeVideoModal state in constructor', this.state)
        console.log('YoutubeVideoModal props in constructor', this.props)
    ...
    handleUrlMetaData(data, index) {
        ...
        if (!data || !data.ogTitle || !data.ogDescription || !data.ogImage || !data.ogImage.url) {
            ...
            links[index] = Object.assign(links[index], { loading: false, valid: true, errorMsg: this.defaultErrorMsg })
    ...
    handleLinkChange(e, index) {
        ...
        if (this.state.links.length < this.elementLimit) {
            if (this.validateYoutubeURL(link)) {
                links[index] = { url: link, valid: true, loading: true, errorMsg: this.defaultErrorMsg }
            } else {
                cards[index] = {
                    disabled: true,
                    id: index + 1,
                    ...
                    <div className='col-6' style={{maxHeight: '65vh', overflowY: 'scroll'}}>
                        {
                            this.state.links.map((link, index) => (
                                    <div>
                                        ...
                                        <div style={{color: 'green'}}>{link.valid && !link.loading ? '*Youtube video link is valid.' : ''}</div>
                                        <div style={{color: 'red'}}>{!link.valid && !link.loading ? `*${link.errorMsg}` : ''}</div>
                                        <div style={{marginBottom: '30px', color: 'green'}}>{link.loading ? '*Retrieving youtube video meta data.' : ''}</div>
                                    </div>
                                ))
                        }
                        {/* {
