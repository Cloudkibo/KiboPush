...
class templatePolls extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps (nextProps) {
    if (nextProps.polls && nextProps.count) {
      if(nextProps.polls.length > 0 && this.state.pageNumber > 1 ) {
        this.setState({lastPollTemplateId: nextProps.polls[0]._id})
       }
       else {
         this.setState({lastPollTemplateId: 'none'})
       }
    ...
    if(nextProps.polls && nextProps.polls.length === 0 && nextProps.count > 0) {
      if (this.state.pageNumber > 0) {
      this.props.loadPollsListNew({last_id: this.state.lastPollTemplateId, number_of_records: 5, first_page: this.state.pageNumber < 2 ? 'first': 'previous', filter: this.state.filter, filter_criteria: {search_value: this.state.searchValue, category_value: this.state.filterValue}})
