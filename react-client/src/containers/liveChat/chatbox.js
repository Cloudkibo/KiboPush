...
class ChatBox extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.socketData && nextProps.socketData.subscriber_id === nextProps.currentSession._id) {
      if (nextProps.socketData.subscriber.waitingForUserInput && nextProps.currentSession.waitingForUserInput.componentIndex === -1) {
        this.setState({waitingForUserInput: false})
      } else if (nextProps.socketData.subscriber.waitingForUserInput && nextProps.currentSession.waitingForUserInput.componentIndex !== -1) {
        this.setState({waitingForUserInput: true})
      }
    }
          ...
          <div style={{padding: '1.3rem', borderBottom: '1px solid #ebedf2'}}>
            <button style={{backgroundColor: 'white'}} className='btn'>Status: {this.props.currentSession.is_assigned ? 'Assigned' : 'Unassigned'}</button>
            {
              ...
              ? <div style={{float: 'right'}}>
                ...
                ? <i style={{cursor: 'pointer', color: '#212529', fontSize: '25px', marginRight: '5px'}} data-target='#pendingResponse' data-toggle='modal' onClick={() => this.showDialogPending(false)} data-tip='Remove Pending Flag' className='la la-user-times' />
              : <i  style={{cursor: 'pointer', color: '#212529', fontSize: '25px', marginRight: '5px'}} data-target='#pendingResponse' data-toggle='modal' onClick={() => this.showDialogPending(true)} data-tip='Add Pending Flag' className='la la-user-plus' />
                }
