          ...
          if (!sub) {
            for (let i = 0; i < bot.payload.length; i++) {
              // if (bot.payload[i].intent_name === intent.value) {
                ...
                //   'Question': temp._text
                // }
              console.log('Exists: ', bot.payload[i].questions.indexOf(temp._text) > -1)
            ...
            if (witres.statusCode !== 200) {
              logger.serverLog(TAG,
                `Error Occured in deleting Wit ai app ${JSON.stringify(
                  witres.body)}`)
              var witBody = JSON.parse(witres.body)
              console.log('Errors', witBody.errors)
              if (witBody.errors) {
                witBody.errors.forEach((error) => {
                  if (error === 'Could not find app.') {
                    Bots.remove({
                      _id: req.body.botId
                    }, function (err, _) {
