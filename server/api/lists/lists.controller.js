    ...
    (err, companyUser) => {
      ...
      if (req.body.first_page) {
        ...
        Lists.aggregate([
          ...
          Lists.find([{$match: {$and: [{companyId: mongoose.Types.ObjectId(companyUser.companyId)}, {_id: {$lt: mongoose.Types.ObjectId(req.body.last_id)}}]}}, {$sort: {datetime: -1}}]).limit(req.body.number_of_records)
          .exec((err, lists) => {
            if (err) {
              return res.status(500).json({
                status: 'failed',
