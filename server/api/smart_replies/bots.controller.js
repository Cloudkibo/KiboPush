...
function getWitResponse (message, token, bot, pageId, senderId) {
  ...
  request(
    ...
    (err, witres) => {
      ...
      logger.serverLog(TAG,
        `Response from Wit AI Bot ${JSON.stringify(JSON.parse(witres.body))}`)
      if (Object.keys(JSON.parse(witres.body).entities).length === 0) {
        logger.serverLog(TAG, 'No response found')
        Bots.findOneAndUpdate({_id: bot._id}, {$inc: {'missCount': 1}})
          .exec((err, dbRes) => {
            if (err) {
              throw err
            } else {
      ...
      if (intent.confidence > 0.55) {
        ...
        Bots.findOneAndUpdate({_id: bot._id}, {$inc: {'hitCount': 1}})
          .exec((err, dbRes) => {
            if (err) {
              throw err
            } else {
...
function sendMessenger (message, pageId, senderId) {
  let messageData = utility.prepareSendAPIPayload(
    ...
    {
      'componentType': 'text',
      'text': message + '  (This is an auto-generated message)'
    }, true)
...
function trainingPipline (entities, payload, token) {
  for (let i = 0; i < entities.length; i++) {
    request(
      {
        'method': 'DELETE',
        'uri': 'https://api.wit.ai/entities/intent/values/' + entities[i] +
        '?v=20170307',
        headers: {
          'Authorization': 'Bearer ' + token
        }
      ...
      (err, witres) => {
        ...
        logger.serverLog(TAG,
          `Response from Training Pipeline ${JSON.stringify(witres)}`)
      })
  }
}
      ...
      if (err) {
        ...
        if (witres.statusCode !== 200) {
          ...
          return res.status(500)
            .json({status: 'failed', payload: {error: witres.body.errors}})
        } else {
          logger.serverLog(TAG,
...
exports.edit = function (req, res) {
  ...
  Bots.update({_id: req.body.botId}, {payload: req.body.payload},
    (err, affected) => {
      if (err) {
        return res.status(500).json({
          status: 'failed',
...
exports.status = function (req, res) {
  ...
  Bots.update({_id: req.body.botId}, {isActive: req.body.isActive},
        ...
        if (err) {
          ...
          if (witres.statusCode !== 200) {
            ...
            Bots.remove({
              ...
              if (err) {
                return res.status(500)
                .json({status: 'failed', payload: err})
              }
              return res.status(200).json({status: 'success'})
            })
